...
public class TextInputFileImpl extends AbstractInputFile {
	...
	public List<Request> readFile() throws BarsException {
		Logger logger = Logger.getLogger(TextInputFileImpl.class);
		...
		try {
			...
			while (br.ready()) {
				...
				errorLine++;
				java.util.Date date= new SimpleDateFormat("MMddyyyy").parse(line.substring(2, 10));
				request.setStartDate(new Date(date.getTime()));

				errorLine++;
				date= new SimpleDateFormat("ddMMyyyy").parse(line.substring(10, 18));
